...
type WorkoutPlanWithDaysAndSessions = Prisma.WorkoutPlanGetPayload<{
  include: {
    workoutDays: {
...
type WorkoutSession = Prisma.WorkoutSessionGetPayload<Record<string, never>>;
...
export class GetStats {
  async execute(dto: InputDto): Promise<OutputDto> {
    ...
    const workoutPlan: WorkoutPlanWithDaysAndSessions | null =
    ...
    const sessions: WorkoutSession[] = await prisma.workoutSession.findMany({
      where: {
        workoutDay: {
    ...
    const completedSessions = sessions.filter(
      (s: WorkoutSession) => s.completedAt !== null
    );
    ...
    const totalTimeInSeconds = completedSessions.reduce(
      (total: number, session: WorkoutSession) => {
  ...
  private async calculateStreak(
    ...
    const completedDates = new Set(
      allSessions.map((s: { startedAt: Date }) =>
