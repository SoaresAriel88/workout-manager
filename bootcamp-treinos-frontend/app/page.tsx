...
import { getHomeData, getUserTrainData } from "./_lib/api/index";
...
export default async function Home() {
  ...
  const [homeData, trainData] = await Promise.all([
    getHomeData(today.format("YYYY-MM-DD")).catch((e) => {
      console.log("homeData error:", e);
      throw e;
    }),
    getUserTrainData().catch((e) => {
      console.log("trainData error:", e);
      throw e;
    }),
  ]);

  if (homeData.status !== 200 && homeData.status !== 404) {
    console.log("homeData status:", homeData.status, "data:", homeData.data);
    throw new Error("Failed to fetch home data");
  }

  const homeDataSuccess = homeData.status === 200 ? homeData.data : null;

  const needsOnboarding =
    !homeDataSuccess?.activeWorkoutPlanId ||
    (trainData.status === 200 && !trainData.data);
  if (needsOnboarding) redirect("/onboarding");

  const { todayWorkoutDay, workoutStreak, consistencyByDay } = homeDataSuccess!;
